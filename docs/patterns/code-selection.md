...
Per D-053, **`codes.codebook`** hops between the surfaces without closing anything: companion closed, open and focus its search; focus in the panel, jump to the companion search; focus in the companion, jump to the panel search. Paired with `codes.focusSearch`, which always returns to the panel, the two chords are the round trip. The companion is a labeled region, "Codebook", and the two search fields carry distinct accessible names, "Search codes" and "Search codebook", so the user always knows which search they are in. The button shows the chord.
...
**Amended by D-070 and restored by D-073.** D-070 removed "Create a code" from this list, on the reasoning that creation belonged at the failure point rather than in standing chrome. D-073 puts it back after the codebook region: the discoverability cost was immediate, and a coder who does not know the empty-search route cannot find it. The empty-search offer stays as an accelerator into the same flow — see *Propose a code* below.
...
**Amended by D-070, restored by D-073: two routes, one flow.** The standing Create code disclosure collects the name; the empty search result proposes the query as one press. Both produce the same record with the same provisional semantics, and each announces its own sentence, because they leave the coder in different places — the accelerator returns them to the search field that now finds the code, and the form collapses to its row.

The empty-name refusal below applies to the form, which can reach that case. The accelerator cannot: the empty state renders only on a query already found non-empty. Neither can create a name another code already has, case-insensitively — the accelerator because its search covers proposals and finds one rather than offering to make a second, the form because the shared creation flow refuses and says so.
...
Per D-070 as amended by D-073, two routes into one creation flow. The standing route: D-039's collapsed Create code disclosure, restored to the fixed region order after the codebook region — a single row with a plus affordance, expanding to name and definition fields with focus moving to the name field, collapsing back to the row. Available to every role at every phase. The accelerator: per D-070, When a search in the coding panel yields no results, the empty state offers one action: "Propose '[query]' as a new code". Activating it creates the code as provisional, name from the query, checks it for the current capture, returns focus to the search field, and announces the result discretely ("Proposed and checked: [name]"). The affordance renders only in the empty-result state, so the panel carries no standing chrome for it. Provisional codes are marked in both channels wherever they render and are fully usable until resolved on the Codebook page.
