...
- Landmarks: one `banner`, one labeled `navigation` for project-level navigation, one `main`, and labeled `region` elements for the transcript, the code panel, the companion codebook, and the review workspace. The companion is a region per D-053, so a browse-mode user reaches it by landmark and its family cards by heading without knowing the hop chord at all — the chord is the fast route, not the only one. Amended by D-068, which removed the application-level `navigation`: it wrapped a single link, so landmark-jumping to the banner arrived at the wrapper rather than at anything in it. A navigation landmark marks a navigation system, and a landmark whose whole content is one link costs a stop on every visit to say nothing. D-068 did not carry the amending clause this change needs, so it is recorded here by the task that built it.
...
8. Reflow at 400% with no horizontal scrolling, run once more at maximum reading scale per D-056 and D-061 — on every route, not the transcript alone, since the scale now reaches all reading content
