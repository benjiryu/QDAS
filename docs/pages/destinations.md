...
**Session controls.** At the very end of the sidebar, per D-072: a disclosure collapsed by default and marked as prototype scaffolding, holding the project phase select and the between-participants reset. Last in reading order, present on every route including `/projects`, and focus stays on its button across the toggle. The reset asks first, clears every store that carries the session's work, and returns to the projects list so the surfaces holding it remount. Scaffolding, per the D-056 addendum: participants should never need it and product features never live here.
