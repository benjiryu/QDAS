...
**The filter list.** Ordered per D-062: families alphabetical by top-level code name, then depth-first within a family with siblings alphabetical — a deliberate, page-scoped divergence from canonical order, since this is a lookup surface. Own view: only codes the coder has used, own-counts. Project-wide view: every code with an active assignment in the project, team-wide counts. Counts count active assignments only, exclude superseded, and sit adjacent to the code name, fused into the control's accessible name ("Water access, 20"). Hierarchy per the D-062 amendment: all pills left-align flat, level carried by fill treatment — parent solid-filled in the family hue, child tinted lighter, grandchild white-filled with colored outline — a channel that survives grayscale; programmatically, D-054 reused — pure name and count in the accessible name, lineage in the description. A child with unused ancestors renders normally, its description naming them. The "All codes" pill is plain gray. Session-evidence flag: if magnification participants misread levels from fill treatment alone, a stronger non-color channel returns. "All codes" heads the list as default. Selected state is border plus bolded count, per the style guide component, never color alone.
