...
**The filter list.** Ordered per D-062: families alphabetical by top-level code name, then depth-first within a family with siblings alphabetical — a deliberate, page-scoped divergence from canonical order, since this is a lookup surface. Own view: only codes the coder has used, own-counts. Project-wide view: every code with an active assignment in the project, team-wide counts. Counts count active assignments only, exclude superseded, and sit adjacent to the code name, fused into the control's accessible name ("Water access, 20"). Hierarchy per D-062: indentation by level reinforced by family shading (parent filled in the family hue, descendants in lighter shades), never color alone; programmatically, D-054 reused — pure name and count in the accessible name, lineage in the description. A child with unused ancestors keeps its full depth, its description naming them. "All codes" heads the list as default. Selected state is border plus bolded count, per the style guide component, never color alone.
