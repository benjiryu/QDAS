...
**Built by Task 53, with "name and definition" read as D-046 left it: a name alone.** That phrase reaches back to D-039's original form, and D-046 amended it — "a name is the whole form. Proposing a code happens mid-coding, and two fields of prose at that moment is a codebook entry demanded in the middle of reading a transcript." This decision does not mention D-046, so the reversal reads as inadvertent rather than intended; section 7 still carries the amendment unstruck. A name alone is also what makes "one flow" literally true, since the accelerator creates from a query and has no definition to offer, and D-070's Accept already collects one at the unhurried moment.

*Two sentences, one flow.* The shared core creates the record, checks it and stays silent; each door announces once. The accelerator says "Proposed and checked: [name]" and returns to the search field, which now finds the code; the form says section 7's sentence with the pending count and collapses to its row. What is shared is the record, the checking and the provisional semantics.

*One thing the restoration added.* The form can reach a case the accelerator cannot: a name another code already has. The accelerator's search covers proposals, so an existing name is found rather than offered — the guard Task 50 built. The form has no such protection by construction, so the shared core refuses a duplicate name case-insensitively, matching the rule D-070 gave the codebook editor, and the form says so rather than failing silently.

*And the pattern document contradicted itself.* This decision's commit amended the "Propose a code" section and left section 3 saying "Create a code is gone from this list" and section 7 saying "there is no form". Both are amended by the task that built it.
