...
Progress: v0.1 complete and tagged. Phase 4 defines v0.2. Tasks 18 through 28 have landed, 28r brought the Codebook page to D-047, and 28a has landed the companion; Task 29, the Coded data page, is next. Open findings, none blocking: the D-033 narrow-width sidebar disclosure is unbuilt (Task 27); search behaves differently in the panel and on the Codebook page, deliberately and per section 1 (Task 28); the fixture's colour tokens no longer describe their hues, so a name map in `familyHues.ts` carries what the swatch is called, guarded by a drift test (Task 28r); the panel's codebook region no longer has a heading, since D-048 made it the Open Codebook button (Task 28a); and the unnamed dialog on the context menu popover is still open (Task 26). D-048 closed the Task 27 finding about the D-044 journey: the companion removes its necessity.
