...
Progress: v0.1 complete and tagged. Phase 4 defines v0.2. Tasks 18 through 29 have landed, including 27r, 28r and 28a, and Tasks 30 through 37 have landed, 31 to 37 out of order; the sequence is complete. **The session gate D-065 opened is closed: the shortcuts help is built (Task 45).** Open findings, one of them for the research team rather than the build: **R-2 still reads "No overlap threshold in v0.1. Exact boundaries are preserved and differences are presented rather than scored", and D-066 introduces an overlap threshold in v0.1. Compatible in substance — nothing is scored, R-3's no-IRR rule is untouched, boundaries are still stored to the character — but the register and the build now say different things, and amending a resolved research question is the team's, so it is recorded under D-066 and left (Task 44)**. Other findings, none blocking: every route into an existing excerpt now opens the code panel rather than the isolated note panel, focus landing on the note where the excerpt carries one — this amends D-055's Open note paragraph and the click route, and closes the split recorded as a finding when Task 36 built it, where a note-only excerpt could gain codes only through `excerpt.open`; `excerpt.note` on a fresh capture and both Notes page routes are untouched (routing change); the change would have removed the only way to delete a note from the transcript, D-055 having made emptying the field the delete and that affordance living in the panel being replaced, so it came across — the code panel's save deletes the coder's own note when the field is emptied, and Save becomes available on a reopened excerpt whose own note was cleared, narrowly, since making it available on any reopened excerpt would have turned unchecking every code into the back route to deletion D-030 forbids (routing change); the D-063 browser test asserting the blue while a panel addresses a note-only range now runs through the code panel, the mechanism having changed underneath it — reopening moves the excerpt machine where a note edit did not — while what the reader sees is the same (routing change); D-073 says the restored disclosure expands "to name and definition", which reaches back to D-039's original form — D-046 amended that to a name alone and section 7 still carries the amendment unstruck, so the reversal reads as inadvertent and the form is a name alone, which is also what makes "one flow" literally true (Task 53); the D-073 commit amended the pattern's "Propose a code" section and left sections 3 and 7 saying the disclosure was gone and there was no form, so the document contradicted itself in two places until this task amended them (Task 53); the form can reach a duplicate-name case the accelerator cannot, its search covering proposals, so the shared creation core refuses a duplicate case-insensitively per the rule D-070 gave the codebook editor and the form reports it (Task 53); each door announces its own sentence once and the shared core stays silent — a test counting announcements rather than reading the last one, since an announcement left in the core doubles the accelerator's and reading the last would not notice (Task 53); **D-072's premise is wrong about the build — it says the phase "was writable only by tests, so a lead could never reach D-070's editing gate in the running prototype", and a "Project phase" select had been on the Coded data page since Task 29, with `asEditingLead` driving a real browser through it to reach that gate. The real gap was that the control sat on one page rather than being reachable from every route; the correction is recorded under D-072 (Task 52)**; the reset clears six stores rather than the one D-072 names, since `clearSimulatedSession` discards no coding work and a confirmation guarding it alone would warn about a loss that does not happen (Task 52); confirming navigates to the projects list, because the transcript workspace holds the session's work in component state and writes through, so a reset underneath it would write the last participant's excerpts back on the next keystroke (Task 52); the Coded data scenario region is removed rather than duplicated, two "Project phase" controls on one page being the collision Task 51 had just resolved for Role (Task 52); the codebook version bump D-070 put at the phase boundary had no test at all until now — Task 49 built it and Task 52 moved its only caller, and neither would have failed had the phase stopped routing through `changePhase` (Task 52); the reset confirmation returns focus to its trigger, which the code panel's delete confirmation it copies does not do (Task 52); a select labelled "Role" already existed in the prototype-support surface and the sidebar renders on every route, so D-071's switcher would have put two controls with one accessible name on one page — the support surface's is "Simulated role" now, per D-053's precedent for the two search fields (Task 51); reviewer left that surface too, since D-071 leaves it unoffered in the sidebar and a native select whose value matches no option renders as its first, so the sidebar would have claimed "AFB Researcher" while the role was reviewer — the cost is that the prototype can no longer put a participant in the reviewer-during-independent-coding case `resolveCodedDataView` reasons about (Task 51); the simulated-session store had to become subscribable, a render-time read having been enough only while the one role control lived on another route and every switch meant a remount (Task 51); the sidebar select needs its own colours and its own focus ring, the nav redefining its text to white for the blue it paints on and scoping the existing ring to `a` (Task 51); three silent failures were found while building the coder half and are closed with it — the transcript rail dropped a provisional's pill while the turn description counted the assignment, so a turn said "1 code" and drew nothing at the moment right after saving; Coded data lost the whole row rather than the pill, an excerpt coded only provisionally being filtered out of the results and out of the count; and Notes rendered such an excerpt as note-only (Task 50); `code.status` was read by no component anywhere, so every surface told a provisional apart by grey, under two stylesheet comments claiming it was marked in words — the mark now rides the D-054 description channel on a code row, never the name D-051 pins, and sits as text after the heading on a codebook record so a deep link arrives with it (Task 50); the panel's search covers proposals now, without which a coder searching for a code they had proposed would have been offered the chance to propose it twice (Task 50); D-039's Create code disclosure and item 8 of its fixed region order are removed, code-selection.md sections 3 and 7 amended to match (Task 50); accepting a provisional left it listed in both the canonical list and the provisional section, a Task 49 defect this task closes — that task tested the code arrived and never that it left (Task 50); editing an existing canonical code works and has no entry point — destinations.md section 1 describes only Create and Accept, and a control on every card is a visible change to a surface D-047 keeps read-only, so the capability is reachable through Accept and the placement is the team's (Task 49); the parent field is a native `select`, which maps to role `combobox` and is what three other fields here already use, at the cost of the D-054 lineage riding in each option's text rather than in a description, an `option` having no description channel (Task 49); a new code's canonical index is a fraction between its parent's subtree and the next code outside it, since the seeded order is dense and global and a literal append would renumber thirty-six records — and only the flat order is affected, the cards being built from the tree, which is why the domain test is the one that bites and the page test was retargeted to what it can actually see (Task 49); three of the seven unclaimed hues are offered, violet, pink and rose, the other four measuring under 3:1 on white and having no business in a picker on this instrument; the three new tokens are named for the ramp they draw, unlike the six legacy ones (Task 49); `resolveEscape` gained a `codeEditor` layer between the help and the code panel, because this panel's Escape discards and is the one that least deserves to race (Task 49); Coded data still never merges provisional codes, a gap that predates this and which D-070 widens — a session-created provisional carrying an assignment resolves to nothing there (Task 49); the "Note:" prefix is scoped to excerpt entries, which D-069 implies through its two-voices reasoning and does not state — a file-wide or project note has one voice, and its text renders inside the button that opens it, so a prefix there would have landed in an accessible name the D-058 addendum composed deliberately and which a test pins exactly (Task 48); the `blockquote` needs `margin: 0` against the user agent's `1em 40px`, eighty pixels of horizontal margin in a 320px column being the reflow failure contract 2.5 forbids, and long words need breaking now that nothing truncates — both held by a browser test, since jsdom has no layout (Task 48); the Notes page's excerpt entry is unreachable in the browser without writing one, every seeded excerpt note belonging to another coder, and the session store is in memory, so the new browser tests reach the page by following the sidebar rather than by `goto` — a full load would have thrown the work away and left them asserting against a page with no excerpt entry on it (Task 48); nothing tested the entry's structure before this, no assertion anywhere touching the disclosure, the truncation, or the excerpt's rendered text, so this task's tests are coverage rather than repair (Task 48); D-068 removed a landmark the accessibility contract requires — section 2.1 asked for an application-level `navigation` beside the project-level one — without carrying the amending clause D-051 carries for the same section, so the contract was amended by this task and the omission recorded rather than passed over, the contract outranking the decision log in this repository's own source-of-truth order (Task 47); two tests held the old structure as their assertion, the shell's landmark count and the list-naming test's name for the wrapper's list, and both now hold the new one — which is what the shell test's own header means by "these assertions are the contract, not a snapshot of the current markup" (Task 47); `.nav-list` was used by nothing else and went with the element it styled (Task 47); D-067's "no indicator renders where the viewer cannot read the note, as today" was describing something this page did not do — the note indicator was derived from every note handed to the derivation, with no author, `visibility` or `status` rule, and the page concatenated seeded and session notes where the Notes page shadow-merges them by `noteId`, so a deleted note left the seeded original standing beside its own tombstone and the row kept saying "Has a note". Closed rather than carried, since a disclosure built on that flag would have revealed the text of a deleted note: one shared `mergeSessionNotes` for both pages, and one domain predicate `canReadNote` (Task 46); who may read another coder's note was chosen rather than found, no decision stating it — readable in the project-wide view where R-4 has lifted and never when `private`, because the Notes page's own-notes-only rule would have emptied the lead's review view of all five seeded note indicators (Task 46); `turnCoding` derives the transcript rail's note indicator with the same absence of any filter, and `canReadNote` is the predicate it wants, but adopting it changes what the rail shows and is a different surface with its own tests (Task 46); the row's link is no longer wrapped in a paragraph, so it and the disclosure button are literally siblings rather than cousins, which is what D-067's structural rule says (Task 46); `CodedResult.hasNote` became `noteText: string | null`, which makes "no indicator where the viewer cannot read the note" structural rather than a rule to remember (Task 46); D-067's constant name means a page of rows presents many buttons all called "Note", told apart only by the row around them — explicit in the decision, followed here, worth watching in a session (Task 46); the fixture's only at-or-above-threshold pair sat at exactly 0.500, which was luck rather than design — the four other seeded pairs measure 0.250 to 0.286, so tuning the provisional threshold up by a hundredth would have left the prototype with nothing to demonstrate; a sixth pair at 0.800 was seeded and every pair's measured ratio is now pinned by a test (Task 44); "touches" is `excerptSegments`, so a boundary sentence the offsets cover zero characters of counts toward both numerator and denominator — consistent with the count the capture announcement already speaks, and deliberately not `excerptText`'s narrower rule, which drops such a sentence (Task 44); only excerpts that are themselves rows can trigger the line, so a note-only or fully superseded excerpt over the same passage is silent — right for a line that says "coded", and worth watching if participants read it as "somebody else is working here" (Task 44); several names read alphabetically and join with commas, neither of which D-066 specifies (Task 44); the project-wide gate beside the comparison is unreachable, the own view's coder filter running first, so removing it fails no test — kept anyway and labelled as such in the derivation, since R-4 should be held by something that says so if `visibleExcerpts` ever widens (Task 44); no excerpt is seeded for the current coder, that being the deliberate change Task 30 flagged (Task 44); the help is reached from a "Keyboard shortcuts" control in the banner as well as from its chord, because a surface teaching chords that is itself reachable only by chord would have left the gate open under a different name; the control sits after the application navigation, which adds one tab stop before `main` on every route (Task 45); the help is reached from a "Keyboard shortcuts" control in the banner as well as from its chord, because a surface teaching chords that is itself reachable only by chord would have left the gate open under a different name; the control sits after the application navigation, which adds one tab stop before `main` on every route (Task 45); `resolveEscape` returns which layer owns Escape rather than a `Command`, since a `help.close` command would need a row in both binding tables where Escape already belongs to `codes.close` and the collision guard would reject the pair (Task 45); the help's Escape handler calls `stopImmediatePropagation`, because the code panel's handler is on `document` too and ordinary propagation does not stop a listener on the same node — which leaves an ordering requirement, that the shell's listener attaches before the panel's, held by the test that takes the help above an open panel (Task 45); the help's open flag reaches `useCodePanel` through a module store rather than a prop or a context, the shape the reading scale already uses here (Task 45); the help's scroll region is focusable and the code panel's is not, since this one is text end to end and a scrollable region with nothing tabbable inside it can only be read with a mouse — axe reported it, WCAG 2.1.1 (Task 45); the labels beside each chord are authored in the component, the table holding no human-readable names, so the words are the build's and the keys are the table's, and a test asserts every command in the union has a row (Task 45); the note panel's exit is listed under both its own group and the code panel's, being one chord with one meaning that a coder should not have to know is shared (Task 45). Earlier findings: the sidebar's pill shape is now defined once on `.project-nav__link` and the two `--radius-pill` overrides are deleted, so hover and the current destination are literally the same shape as D-059 said they were (Task 43); `User` gains a `title` field, since the D-059 addendum requires the sidebar's title block to read the seed record and neither `displayName` nor `role` is a title — domain-model.md follows (Task 43); two browser tests used the current pill's radius as a channel distinguishing it from the others, which the unification removes, so they now assert the shared shape and the luminance step instead (Task 43); the captured band now needs its sentence's state attribute in the selector to outrank the coded wash, which is written at equal specificity in another stylesheet — the precedence is intended, a range a panel is addressing outranking what it already carries, and it is spelled out where it is relied on (Task 41); `note.open` paints its range as a display-only addressed range rather than by moving the excerpt machine, since a note edit is not a capture and moving it would make the excerpt commands refuse (Task 41); the boundary bars took `--color-blue-100`, which D-063 does not name, measured at 6.07:1 on the band where a non-text indicator needs 3:1 (Task 41); the Coded Data filter list has no visual level channel at all since D-064 removed the pills — D-062 gave level indentation, the amendment replaced it with fill treatment, and both are gone, so hierarchy reaches only a screen reader through the lineage description; recorded with a session-evidence flag (D-064); the family underline uses shade-1 and stays faint for four families, 1.69:1 yellow, 2.00:1 light green, 2.37:1 sea green, 2.41:1 orange, pinned per family by a browser test — not a contrast failure, since the name is text and grouping is carried by position, but a real limit, and the opposite of the transcript's choice under D-041 (D-064); the filter search is new behaviour destinations.md section 2 did not specify, now amended (D-064); the D-062 amendment's claim that "the gray row background provides the shape contrast" at the grandchild level is measured false — a white pill is 1.21:1 on the selected row's grey and has no boundary on the white page behind an unselected one, so the pill's shape rests on an outline that is 1.69:1 for yellow, 2.00:1 for light green, 2.37:1 for sea green and 2.41:1 for orange, all under 3:1. Shipped as designed under the amendment's own session-evidence flag, with the figures recorded under the decision and pinned by a browser test; the D-054 lineage description carries level programmatically regardless (Task 42); D-062 asks for parent, child and grandchild pills in three progressively lighter shades, but only two of the family shades take black text — shade-1 measures 3.43:1 at worst and fails 4.5:1 for violet, blue, purple, dark green and rose, against 6.95:1 for shade-2 and 13.34:1 for shade-3 — so the third level is unfilled with the family border it already carries, and a browser test measures every pill's contrast off the page rather than trusting the token audit, which speaks only for shade-2 (Task 40); the lookup ordering is one path comparison over `[...lineage, name]` and is scoped to this page, with `byCanonicalOrder` still ordering the codebook, the panel and each result row's codes (Task 40); the reading scale is one root custom property now rather than a font-size on the transcript, so it reaches pages with no transcript on them, and the preference is a subscribed module store rather than a context — a provider would have required one above every test that mounts a single region (Task 39); the filter rows on Coded data and Notes split the code or source name from its count, since D-061 classifies the two apart and they were one string (Task 39); a rail pill needs `min-width: 0` as well as a word break to stay inside 320px at maximum scale, a flex item's automatic minimum being its min-content width (Task 39); the context menu's preview highlight was reverted per D-060, an uncaptured range never wearing an application visual, and the selection restoration it needed went with it; the real cause of the lost highlight was measured rather than the one D-060 names — React Aria put `inert` on the document behind the menu, and inert content has its selection neither painted nor reported, so the menu is non-modal now and the authored `::selection` handles the inactive repaint that remains, both halves load-bearing and each covered by a test that fails without it (D-060 addendum); jsdom cannot test that the selection survives, since it collapses one whenever anything takes focus, so the acceptance criterion lives only in the browser suite; "Code selection" is named "Assign code" on the strip and in the menu together, keeping D-028's word-for-word parity; the banner moved into the shell grid's right column so the sidebar can be flush to the viewport's top, DOM order and landmarks unchanged (Task 38); the Figma sidebar's home icon and user name above the file group are not built, being identity chrome the shared Sidebar rule's fixed content order does not include and prototype-scope keeps simulated, and its Overarching Themes destination stays unfollowed per D-017 and D-047 (Task 38); the current destination's pill text moved from black to the dark blue token, 8.59:1 on white, and hover now wears the same pill, which D-059 accepts and forbids diverging from (Task 38); the Notes page's excerpt entry link reads "Note on [speaker], [source]" with the source visually hidden inside it rather than supplied by `aria-label`, so the visible label stays contained in the accessible name; the speaker moved out of the excerpt disclosure to the link, since rendering it in both would be the duplication the D-058 addendum removes (Notes page, one channel per fact); the Notes page shows no excerpt note on a fresh session, because every seeded excerpt and excerpt note belongs to the second coder and R-4 hides them — giving the scope a seeded example means adding an own excerpt, which moves Coded data's counts and its empty state, so it is a change for the team to make deliberately (Task 30); a seeded own note is edited by shadowing it in the session store rather than mutating the fixture, since the fixture is a module constant (Task 30); the scope field appears only when creating a note, as D-058 specifies the field for creation and says nothing about moving a note between scopes (Task 30); the Notes filter reuses the Coded data filter store under a `notes` view key rather than gaining a store of its own (Task 30); D-056 gives the text size range and not the step, so 25 points is the build's choice, seven stops from 100 to 250 (Task 37); the transcript's two layout crossovers are container queries now rather than one media query and one container query, because a viewport breakpoint cannot see the text size and at 250 percent it put a 360px speaker column beside a prose track with nothing left (Task 37); long words break at 320px and maximum size, since a single word can be wider than the viewport there (Task 37); the text size control is unreachable while the code panel is open, that panel being modal, which is the same aria-hidden consequence recorded as V-3 (Task 37); `note.open` has no strip control, so its only pointer-free route is the chord — the rail icon is the pointer twin D-055 names, and being inside the aria-hidden rail it carries `tabindex="-1"`. CLAUDE.md's "every keyboard command has a visible control" was removed in the same session rather than excepted; excerpt-selection.md section 4 still says each command in its table has a visible strip control, and `excerpt.open` and `note.open` both now contradict it, so that sentence is the one left to settle (Task 36); clicking a note-only excerpt opens the note panel while `excerpt.open` still reopens it for coding, which is deliberate rather than an inconsistency: that command is the only way a passage noted before it was coded can gain codes, since re-capturing the range makes a second excerpt. Section 3 records all three routes; emptying another coder's note deletes nothing and says so, which D-055 does not cover (Task 36); the lineage description was applied to the search results rows as well as the code list, which D-054 does not name, because those rows already showed the same lineage as loose text and would otherwise have said it twice (Task 35); a participant whose screen reader has descriptions turned off hears no lineage at all, which is the cost of putting it under their own verbosity setting rather than in the name (Task 35); the codebook search is now named "Search codebook" on the destination page as well as in the companion, since one component renders both and D-053 names only the companion's (Task 34); the hop announces nothing, leaving the field it lands in to speak for itself (Task 34); `codes.codebook` does nothing with the panel closed, which is how D-053 scopes it (Task 34); reading through a coded passage is now a manual check per screen reader, smoke test item 13, and no automated test replaces it (Task 33); D-052 does not name the note-only range, which is marked on the reasoning recorded under the decision (Task 33); a captured range's mark cannot be reached, because the modal panel `aria-hidden`s the transcript for the whole time a capture exists, recorded as V-3 (Task 33); the D-033 narrow-width sidebar disclosure is still unbuilt, and more visible now that the sidebar is solid blue (Tasks 27 and 27r); the sidebar holds its place with `sticky` rather than `fixed`, which keeps it in the flex row (Task 27r); search behaves differently in the panel and on the Codebook page, deliberately and per section 1 (Task 28); the fixture's colour tokens no longer describe their hues, so a name map in `familyHues.ts` carries what the swatch is called, guarded by a drift test (Task 28r); the panel's codebook region no longer has a heading, since D-048 made it the Open Codebook button (Task 28a); D-049 does not say which view a reviewer gets during independent coding, resolved conservatively to own work (Task 29); the Codebook search now announces its result count, which is new behaviour destinations.md section 1 does not specify (Task 31); panel-open focus could not be reproduced as a defect in Chromium, so the manual VoiceOver finding needs a re-check by whoever found it (Task 31); Task 32 names two things absent from the build — recent codes, removed from the panel earlier, and the Notes list, still unbuilt — and D-051's double-stop could not be demonstrated in Chromium, where every labelled control exposes its label text whether the label wraps or not, so that half needs the VoiceOver re-check (Task 32); and the unnamed dialog on the context menu popover is still open (Task 26). D-048 closed the Task 27 finding about the D-044 journey: the companion removes its necessity.
...
**A behavior change negotiated in a fix session is a decision.** It reaches the decision log, or it will be re-litigated by whoever reads the log in good faith. This is not a formality: note-only excerpts were decided and built during a fix session, never logged, and the superseded sentence left standing in D-042 caused the whole question to be reopened as though it were still open. Recorded at D-055's request, because it cost a round trip to find out.
...
- Plus and minus buttons in the transcript header near the position ribbon,
  ...
  100 to 250 percent. Never in the prototype-support surface that holds the
  role and phase controls; that surface is scaffolding, not product, per the
  D-056 addendum
...
### Task 39. Reading scale extends to all reading content

```
Implement decision D-061. Replace the transcript-scoped font-size with a
root --reading-scale custom property set by the existing header control.
Reading content opts in per the decision's classification: transcript, code
names and pill labels everywhere, codebook definitions, note text and field,
excerpt and note text on destination pages, search input text. Chrome never
scales: sidebar, strip, action buttons, page headings, position ribbon.
Pills move to em-based padding and radius. At 250 percent the panel reflows
vertically with internal scroll and no horizontal panning; checkbox targets
grow with their rows. The 320-width reflow test runs at maximum scale.
...
### Task 41. Confirmed highlight in selection blue

```
Implement decision D-063 per the amended docs/patterns/excerpt-selection.md
section 6. The confirmed-state highlight uses the same blue token as the
author ::selection style from D-060; the purple treatment is removed.
Reopened ranges wear the same blue while their panel is open and return to
their saved treatment on close. Add a test that the capture transition
produces no color change from the native selection appearance, and that
...
### Task 43. Nav pill shape and the user title block

```
Implement the D-059 addendum per the amended docs/pages/destinations.md
sidebar rules. Unify hover and current-page pills on the hover shape, the
rounded rectangle. Add the user title block at the top of the nav landmark:
static text from the seeded user record reading "AFB Researcher", not a
...
### Task 46. Note disclosure on Coded Data rows

```
Implement decision D-067 per the amended docs/pages/destinations.md section
2. The note indicator becomes a disclosure button: constant accessible name
"Note", aria-expanded, revealed region immediately following in DOM order,
focus stationary across toggling, button a sibling of the row's link and
never nested inside it. No live-region announcement. Note text scales per
D-061. No indicator where the viewer cannot read the note. Tests cover the
expanded reading order, the nesting rule, and toggling without focus loss.
When tests pass, commit and update the progress line in the same commit.
Do not push.
```
...
### Task 51. The role switcher in the sidebar

```
Implement decision D-071 per the amended docs/pages/destinations.md sidebar
rules. The title block becomes a native select labeled "Role", options AFB
Researcher (coder) and Qualitative Lead (qualitativeLead), value displayed
as the title. Role change announces discretely through the live region
service and focus stays on the select; gated controls elsewhere update
without taking focus. The select drives the same role state the
prototype-support surface drives, one source of truth, and the two controls
stay in sync. Phase switching does not move. Tests: switching to lead in a
non-coding phase reveals Create new code on the Codebook page without focus
loss; switching back hides it; the announcement fires once per change. When
tests pass, commit and update the progress line in the same commit. Do not
push.
```
